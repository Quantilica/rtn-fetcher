...
def cmd_metadata(
    output: Annotated[
        Path, typer.Option("-o", "--output", help="Diretório para metadata.html e metadata.json")
    ] = _DEFAULT_OUTPUT,
    encoding: Annotated[str, typer.Option("--encoding")] = "utf-8",
    force: Annotated[bool, typer.Option("--force", help="Rebaixar HTML mesmo se já existir")] = False,
) -> None:
    ...
    out_json.write_text(json.dumps(publications, ensure_ascii=False, indent=2), encoding=encoding)
    console.print(
        f"[green]✓[/green] Salvas [bold]{len(publications)}[/bold] publicações em [bold]{out_json}[/bold]"
    )
...
def cmd_download(
    ...
    encoding: Annotated[str, typer.Option("--encoding")] = "utf-8",
    concurrency: Annotated[int, typer.Option("--concurrency", help="Downloads simultâneos")] = 4,
) -> None:
    """Baixar arquivos referenciados no metadata.json."""
    from quantilica_core.http import BROWSER_HEADERS, AsyncHttpClient

    if not metadata.exists():
        console.print(f"[red]Metadados não encontrados:[/red] {metadata}", stderr=True)
        raise typer.Exit(2)
    ...
    async def _run() -> None:
        ...
        tasks = [
            _bounded_download(pub, filename, url, client, output, encoding, semaphore)
...
def cmd_latest(
    ...
    console.print(f"[green]✓[/green] Arquivo RTN mais recente: [bold]{filepath}[/bold]")
...
def cmd_export_excel(
    ...
    for sheet_name, (data_tbl, accounts_tbl, sheet_title) in results.items():
        ...
        for row in data_tbl.iter_rows():
            ...
            period_date = make_period_date(rd.get("year"), rd.get("month"), rd.get("quarter"))
            for col_idx, value in enumerate([f"{sheet_name}|{rd.get('account')}", period_date, rd.get("value")], 1):
        ...
        for row in accounts_tbl.iter_rows():
            ...
            for col_idx, value in enumerate(
                [f"{sheet_name}|{account_code}", sheet_name, sheet_title, account_code, rd.get("account_name"), rd.get("account_level"), *p_values], 1
            ):
...
def cmd_export_sqlite(
    ...
    cursor.execute(
        f"CREATE TABLE rtn_accounts (key TEXT, sheet TEXT, sheet_title TEXT, account_code TEXT, account_name TEXT, account_level INTEGER, {cols_sql})"
    )
    ...
    for sheet_name, (data_tbl, accounts_tbl, sheet_title) in results.items():
        ...
        for row in data_tbl.iter_rows():
            ...
            rd = dict(zip(data_header, row, strict=False))
            period_date = make_period_date(rd.get("year"), rd.get("month"), rd.get("quarter"))
            cursor.execute("INSERT INTO rtn_data VALUES (?, ?, ?)", (f"{sheet_name}|{rd.get('account')}", period_date, rd.get("value")))
        ...
        for row in accounts_tbl.iter_rows():
            ...
            cursor.execute(
                f"INSERT INTO rtn_accounts (key, sheet, sheet_title, account_code, account_name, account_level, {', '.join(hierarchy_cols)}) VALUES (?, ?, ?, ?, ?, ?, {placeholders})",
                [f"{sheet_name}|{account_code}", sheet_name, sheet_title, account_code, rd.get("account_name"), rd.get("account_level"), *p_values],
            )
